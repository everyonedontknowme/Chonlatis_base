...
function OpenMobileAmbulanceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_ambulance_actions', {
		...
		if data.current.value == 'citizen_interaction' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				elements = {
					...
					{label = _U('ems_menu_billing'), value = 'fine'}, --billing
					-- {label = 'พาตัวคนไข้', value = 'drag'},
					-- {label = 'กระเป๋าพยาบาล', value = 'emsbag'},
				...
				if closestPlayer == -1 or closestDistance > 1.0 then
					...
					elseif data.current.value == 'drag' then
						...
						TriggerServerEvent('esx_barbie_lyftupp:lyfteruppnEMS', GetPlayerServerId(player))
						if hasRope == true then
							...
							if distance ~= -1 and distance <= 3.0 then
								...
								TriggerServerEvent('esx_barbie_lyftupp:lyfterEMS', GetPlayerServerId(closestPlayer))		
		...
		-- if data.current.value == 'squad' then
			-- TriggerEvent("meeta_medic:squadMenu")
        -- end
...
function OpenPharmacyMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pharmacy', {
		...
		elements = {
			...
			{label = _U('pharmacy_take', ('ชุดดำน้ำ')), value = 'oxygen_mask'},
			{label = _U('pharmacy_take', ('ยาแก้เครียด')), value = 'antistress'},
