...
ESX.TriggerServerCallback = function(name, requestId, source, cb, ...)
	if ESX.ServerCallbacks[name] then
		...
		print(('[^2es_extended^7] TriggerServerCallback[^3%s^7]: Request ID: %s Player ID: ^3%s^7'):format(name, requestId, source))
	else
...
ESX.SavePlayer = function(xPlayer, cb)
	...
	table.insert(asyncTasks, function(cb2)
		MySQL.Async.execute('UPDATE users SET accounts = @accounts, job = @job, job_grade = @job_grade, `group` = @group, loadout = @loadout, position = @position, inventory = @inventory WHERE identifier = @identifier', {
			...
			['@hparmour'] = json.encode(xPlayer.getHpArmour()),
	...
	Async.parallel(asyncTasks, function(results)
		...
		local saveplayertext = 'Save Player :'..xPlayer.getName()
		TriggerEvent('azael_discordlogs:sendToDiscord', 'saveplayer', saveplayertext, xPlayer.source, '^1')
		if cb then
			cb()
		end
	end)
...
ESX.CreatePickup = function(type, name, count, label, playerId, components, tintIndex)
	...
	Citizen.CreateThread(function()
		Citizen.Wait(Config.ItemDropExpire * 1000)
