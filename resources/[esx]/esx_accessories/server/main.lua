...
AddEventHandler('esx_accessories:saveOutfit', function(label, skin, accessory)
	...
	local count_item = MySQL.Sync.fetchScalar("SELECT COUNT(1) FROM xcjjoeza_accessory WHERE owner = @owner AND type = @type", {
	...
	if count_item >= 100 then
		...
		MySQL.Async.execute('INSERT INTO xcjjoeza_accessory(owner, label, skin, type) VALUES (@owner, @label, @skin, @type)', {
			['@owner'] = xPlayer.identifier,
			['@label'] = label,
