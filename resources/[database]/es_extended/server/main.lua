Citizen.CreateThread(function()
	SetMapName(Config.SetMapName)
	SetGameType(Config.SetGameType)
end)
...
function onPlayerJoined(playerId)
	...
	for k,v in ipairs(GetPlayerIdentifiers(playerId)) do
		if string.match(v, Config.Identifier.identifier --[[ 'license:' ]]) then
			identifier = string.sub(v, Config.Identifier.startAtIndex --[[ 9 ]])
			break
		end
	end
...
function loadESXPlayer(identifier, playerId)
	...
	table.insert(tasks, function(cb)
		MySQL.Async.fetchAll('SELECT accounts, job, job_grade, `group`, loadout, position, hparmour, inventory FROM users WHERE identifier = @identifier', {
			['@identifier'] = identifier
		}, function(result)
			...
			if result[1].hparmour then
				userData.hparmour = json.decode(result[1].hparmour)
			end
	...
	Async.parallel(tasks, function(results)
		local xPlayer = CreateExtendedPlayer(playerId, identifier, userData.group, userData.accounts, userData.inventory, userData.weight, userData.job, userData.hparmour, userData.loadout, userData.playerName, userData.coords)
...
AddEventHandler('playerDropped', function(reason)
	...
	--xPlayer.setHpArmour(GetEntityHealth(ped), GetPedArmour(ped))

	if xPlayer then
		--xPlayer.setHpArmour(GetEntityHealth(ped), GetPedArmour(ped)) --ย้ายจาก 283
	...
	if type == 'item_standard' then
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > xItem.count or xItem.count < 1) then
				...
				local message = "**" .. xPlayer.name .. "** ได้ทิ้ง "..ESX.GetItemLabel(ItemName).." X"..(ItemCount)
		    	TriggerEvent("azael_discordlogs:sendToDiscord", "RemoveItem", message, source, "^2")
	...
	elseif type == 'item_account' then
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > account.money or account.money < 1) then
				...
				local message = "**" .. xPlayer.name .. "** ได้ทิ้ง "..account.." X"..(ItemCount)
		    	TriggerEvent("azael_discordlogs:sendToDiscord", "RemoveMoney", message, source, "^2")
...
AddEventHandler('esx:useItem', function(itemName)
	...
	if count > 0 then
		...
		local message = "**" .. xPlayer.name .. "** ได้ใช้ไอเท็ม "..ESX.GetItemLabel(itemName)
		TriggerEvent("azael_discordlogs:sendToDiscord", "UseItem", message, source, "^2")
	else
