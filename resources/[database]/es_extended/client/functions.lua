...
ESX.ShowNotification = function(msg, flash, saveToBrief, hudColorIndex, customNotify)
	local customNotify = customNotify or {}
	if customNotify == nil or customNotify then
		--exports['mythic_notify']:SendAlert(customNotify.type or 'inform', replaceMsg(msg), customNotify.length or 5500)
		exports['Chonlatis_notify']:MsgNotification(1 , replaceMsg(msg), customNotify.length or 5)
	else
...
ESX.Game.SpawnVehicle = function(modelName, coords, heading, cb)
	...
	Citizen.CreateThread(function()
		...
		DecorSetInt(vehicle, "Player_Vehicle", decorid)
...
--[[
RegisterNetEvent('esx:updatePlayer')
AddEventHandler('esx:updatePlayer', function(ps, st)
	esxPlayers = ps
	print(string.format("Status: %s", st))
end) ]]--

ESX.Game.GetPlayers = function(onlyOtherPlayers, returnKeyValue, returnPeds)
	local players, myPlayer = {}, PlayerId()

	for k,player in ipairs(--[[esxPlayers]]  GetActivePlayers() ) do --for k,player in ipairs(GetActivePlayers()) do
...
ESX.Game.SetVehicleProperties = function(vehicle, props)
	if DoesEntityExist(vehicle) then
		...
		-- 	-- exports["LegacyFuel"]:SetFuel(vehicle, props.fuelLevel+0.00)
		-- 	exports["taeratto_legacyfuel"]:SetFuel(vehicle, props.fuelLevel+0.00)
...
Citizen.CreateThread(function()
	while fontCustom == nil do
		fontCustom = exports['monster_font']:GetFontId('srbn')
		Citizen.Wait(200)
	end
end)
...
ESX.Game.Utils.DrawText3D = function(coords, text, size, font)
	...
	if not font then font = fontCustom or 0 end
...
AddEventHandler('esx:playerLoaded', function(playerData)
	...
	for i=1, #playerData.inventory do
		...
		if item.count > 0 then
			weight = weight + (item.count * item.weight)
		end
		-- Update new by TaerAttO
		Items[item.name] = {
			label = item.label,
			weight = item.weight,
			rare = item.rare,
...
AddEventHandler('esx:addInventoryItem', function(name, count, s, w)
	weight = w
end)

RegisterNetEvent('esx:removeInventoryItem')
AddEventHandler('esx:removeInventoryItem', function(item, count, s, w)
	weight = w
end)
...
ESX.canCarryItem = function(name, count)
	local currentWeight, itemWeight = weight, Items[name].weight
	local newWeight = currentWeight + (itemWeight * count)

	return newWeight <= ESX.PlayerData.maxWeight
end

ESX.canSwapItem = function(firstItem, firstItemCount, testItem, testItemCount)
	local firstItemObject = ESX.getInventoryItem(firstItem)
	local testItemObject = ESX.getInventoryItem(testItem)

	if firstItemObject.count >= firstItemCount then
		local weightWithoutFirstItem = ESX.Math.Round(weight - (firstItemObject.weight * firstItemCount))
		local weightWithTestItem = ESX.Math.Round(weightWithoutFirstItem + (testItemObject.weight * testItemCount))

		return weightWithTestItem <= ESX.PlayerData.maxWeight
	end
...
function replaceMsg(txt)
	...
	return txt:gsub("~%S~+", replacements)
end

RegisterNetEvent("JackBlack:GenHashCar")
AddEventHandler("JackBlack:GenHashCar", function(hashcar)
	decorid = hashcar
end)