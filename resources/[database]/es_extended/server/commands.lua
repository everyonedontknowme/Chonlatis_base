...
ESX.RegisterCommand('setaccountmoney', 'admin', function(xPlayer, args, showError)
	if args.playerId.getAccount(args.account) then
		...
		local message = "**" .. xPlayer.name .. "** ได้เซ็ตเงิน: "..(args.account).." X"..(args.amount).. 'ให้กับ :'..(args.playerId.name)
		TriggerEvent("azael_discordlogs:sendToDiscord", "admingivemoney", message, xPlayer.playerId, "^2")
	else
...
ESX.RegisterCommand('giveaccountmoney', 'admin', function(xPlayer, args, showError)
	if args.playerId.getAccount(args.account) then
		...
		local message = "**" .. xPlayer.name .. "** ได้เสกเงิน: "..(args.account).." X"..(args.amount).. 'ให้กับ :'..(args.playerId.name)
		TriggerEvent("azael_discordlogs:sendToDiscord", "admingivemoney", message, xPlayer.playerId, "^2")
	else
		showError(_U('command_giveaccountmoney_invalid'))
	end
...
ESX.RegisterCommand('giveitem', 'admin', function(xPlayer, args, showError)
	...
	local message = "**" .. xPlayer.name .. "** ได้เสกไอเท็ม: "..(args.item).." X"..(args.count)..' ให้กับ :'..(args.playerId.name)
	TriggerEvent("azael_discordlogs:sendToDiscord", "admingiveitem", message, xPlayer.playerId, "^2")
...
ESX.RegisterCommand('giveweapon', 'admin', function(xPlayer, args, showError)
	if args.playerId.hasWeapon(args.weapon) then
		...
		local message = "**" .. xPlayer.name .. "** ได้เสกอาวุธ: "..(args.weapon).." X"..(args.ammo)..' ให้กับ :'..(args.playerId.name)
		TriggerEvent("azael_discordlogs:sendToDiscord", "admingiveweapon", message, xPlayer.playerId, "^2")
	end
