function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, hparmour, loadout, name, coords)
	local self = {}
	...
	-- add by TaerAttO
	self.hparmour = hparmour

	-- settings multi license
	local ggwpMonsterTaerAttOggwpMyVariblePleaseNotChangeThisIfYouChangeThisImNotSupportYou
	if string.match(self.identifier, 'steam:') then
		ggwpMonsterTaerAttOggwpMyVariblePleaseNotChangeThisIfYouChangeThisImNotSupportYou = self.identifier
	else
		ggwpMonsterTaerAttOggwpMyVariblePleaseNotChangeThisIfYouChangeThisImNotSupportYou = 'license:' .. self.identifier
	end
	-- end settings multi license by TaerAttO

	ExecuteCommand(('add_principal identifier.%s group.%s'):format(ggwpMonsterTaerAttOggwpMyVariblePleaseNotChangeThisIfYouChangeThisImNotSupportYou--[[ self.identifier ]], self.group))
	...
	self.setBank = function(money)
		money = ESX.Math.Round(money)
		self.setAccountMoney('bank', money)
	end
	...
	self.setGroup = function(newGroup)
		ExecuteCommand(('remove_principal identifier.%s group.%s'):format(ggwpMonsterTaerAttOggwpMyVariblePleaseNotChangeThisIfYouChangeThisImNotSupportYou--[[ self.identifier ]], self.group))
	...
	self.setAccountMoney = function(accountName, money)
		if money >= 0 then
			...
			if account then
				...
				local messageDiscord = 'เซ็ตเงิน [' .. accountName .. '] จำนวน [' .. money .. '] เงินทั้งหมดที่มี [' .. account.money .. ']'
				TriggerEvent('taeratto_discord:server:Discord', self.source, messageDiscord, self.identifier)
	...
	self.addAccountMoney = function(accountName, money)
		if money > 0 then
			...
			if account then
				...
				local messageDiscord = 'ได้รับ [' .. accountName .. '] จำนวน [' .. money .. '] รวมทั้งหมดเป็น [' .. account.money .. ']'
				TriggerEvent('taeratto_discord:server:Discord', self.source, messageDiscord, self.identifier)
				self.triggerEvent('esx:setAccountMoney', account)
			end
		end
	...
	self.removeAccountMoney = function(accountName, money)
		if money > 0 then
			...
			if account then
				...
				local messageDiscord = 'เสีย [' .. accountName .. '] จำนวน [' .. money .. '] ยังมีเงินอยู่ [' .. account.money .. ']'
				TriggerEvent('taeratto_discord:server:Discord', self.source, messageDiscord, self.identifier)
				self.triggerEvent('esx:setAccountMoney', account)
			end
		end
	...
	self.addInventoryItem = function(name, count)
		...
		if item then
			...
			self.triggerEvent('esx:addInventoryItem', item.name, item.count, false, self.weight)
		end
	end
	...
	self.removeInventoryItem = function(name, count)
		...
		if item then
			...
			if newCount >= 0 then
				...
				self.triggerEvent('esx:removeInventoryItem', item.name, item.count, false, self.weight)
			end
		end
	end
	...
	-- add by TaerAttO
	self.getHpArmour = function()
		return self.hparmour
	end

	self.setHpArmour = function(hp, armour)
		self.hparmour.health = hp
		self.hparmour.armour = armour
	end
	-- end add
