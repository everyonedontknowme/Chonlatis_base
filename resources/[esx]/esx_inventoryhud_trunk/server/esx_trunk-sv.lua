...
AddEventHandler("esx_trunk:getItem", function(plate, type, item, count, max, owned)
	-- print (plate, type, item, count, max, owned)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)

    if type == "item_standard" then
		local targetItem = xPlayer.getInventoryItem(item)
		TriggerClientEvent("discordbot:re2cars", _source, plate, type, item, count)
		if targetItem.limit == -1 or ((targetItem.count + count) <= targetItem.limit) then
			TriggerEvent("esx_trunk:getSharedDataStore", plate, function(store)
				...
				data = {plate = plate, weight = weight, max = max, myVeh = owned, text = "TRUNK INVENTORY"}
    ...
    if type == "item_account" then
		TriggerEvent("esx_trunk:getSharedDataStore", plate, function(store)
			...
			if (blackMoney[1].amount >= count and count > 0) then
				...
				data = {plate = plate, max = max, myVeh = owned, text = "TRUNK INVENTORY"}
				TriggerClientEvent("esx_inventoryhud:refreshTrunkInventory", _source, data, blackMoney, items, weapons)
			else
				TriggerClientEvent("pNotify:SendNotification", _source, {
...
AddEventHandler("esx_trunk:putItem", function(plate, type, item, count, max, owned, label)
    ...
    if type == "item_account" then
		...
		TriggerClientEvent("discordbot:add2cars", _source, plate, type, item, count)
